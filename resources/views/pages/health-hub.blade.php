@php
    $articles = collect(config('sandwell.articles'));
...
<x-layout title="Health Hub" hero description="Plain-English health guidance from our pharmacists — seasonal, practical and free.">
    <x-page-hero image="consult-room.jpg" alt="A pharmacist talking with a patient">
        <div class="max-w-3xl">
            <p class="reveal text-[11px] font-bold uppercase tracking-[0.14em] text-brand-orange">Health Hub</p>
        ...
        <div class="mx-auto max-w-[1280px] px-6 sm:px-8">
            ...
            <a href="{{ route('health-hub.show', $featured['slug']) }}"
                ...
                <div class="flex flex-col justify-center p-8 md:p-12">
                    ...
                    <span class="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-brand-orange">
                        Read article
    ...
    <section id="weight-loss" class="scroll-mt-24 border-t border-brand-hairline bg-white py-24 md:py-32">
        <div class="mx-auto max-w-[1280px] px-6 sm:px-8">
            <div class="reveal flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
                ...
                <a href="{{ route('weight-loss') }}" class="shrink-0 text-sm font-semibold text-brand-orange hover:underline">Visit the weight loss clinic →</a>
            </div>
    ...
    <section id="tools" class="no-interact scroll-mt-24 border-y border-brand-peach-line bg-brand-peach py-24 md:py-32">
        <div class="mx-auto max-w-[1280px] px-6 sm:px-8">
                ...
                <div class="reveal rounded-[20px] border border-brand-peach-line bg-white p-8">
                    ...
                    <div class="mt-6 flex flex-wrap gap-2">
                        @foreach (range('A', 'H') as $letter)
                            <span class="flex h-9 w-9 items-center justify-center rounded-full bg-brand-navy/5 text-sm font-medium text-brand-navy">{{ $letter }}</span>
                        @endforeach
                    </div>
                </div>
