@php
    ...
    $answers = array_column(array_merge(...array_column(config('pharmacy.faqs'), 'items')), 1, 0);
    $homeFaqs = array_map(fn ($q) => [$q, $answers[$q]], config('pharmacy.faqs_home'));
...
<x-layout hero>
    ...
    <x-page-hero image="hero-consult.jpg" alt="A pharmacist talking with a patient at the counter">
        <div class="max-w-3xl">
            ...
            <div class="no-interact reveal mt-12 flex flex-wrap items-center gap-x-6 gap-y-3 border-t border-white/15 pt-7" style="--reveal-delay:540ms">
                ...
                <span class="text-sm text-white/70">Free delivery over {{ config('pharmacy.free_delivery_threshold') }}</span>
            </div>
        </div>
            ...
            <div class="relative">
                <div data-carousel class="no-scrollbar flex gap-5 overflow-x-auto pb-1">
                    @foreach ([0, 1] as $pass)
                        @foreach ([
                            ...
                            @php
                                $toneClass = ['nhs' => 'text-[#8cc2ef]', 'private' => 'text-brand-moss', 'shop' => 'text-white/55'][$tone];
                            @endphp
                            <a href="{{ $href }}" data-carousel-item @if ($pass) aria-hidden="true" tabindex="-1" @endif
                                class="group relative aspect-[4/5] w-[78vw] shrink-0 overflow-hidden rounded-[28px] shadow-editorial-hover focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-moss focus-visible:ring-inset sm:w-[400px] lg:w-[413px]">
                                ...
                                <span class="absolute inset-x-0 bottom-0 flex h-1/2 flex-col justify-center bg-brand-forest px-8">
                ...
                @foreach ([['prev', 'Previous services', 'left-3 sm:left-5', 'M15 19l-7-7 7-7'], ['next', 'More services', 'right-3 sm:right-5', 'M9 5l7 7-7 7']] as [$dir, $label, $pos, $path])
                    <button type="button" data-rail-{{ $dir }} aria-label="{{ $label }}"
                        class="absolute top-1/2 z-20 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-white/25 bg-brand-forest-deep/70 text-white backdrop-blur transition-all duration-200 hover:scale-105 hover:bg-brand-moss hover:border-brand-moss active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-moss {{ $pos }}">
            ...
            @for ($pass = 0; $pass < 2; $pass++)
                <div class="flex shrink-0 items-center gap-6 pr-6 text-[11px] font-semibold uppercase tracking-[0.12em] text-brand-stone" @if($pass) aria-hidden="true" @endif>
                    @foreach (['Pharmacy First', 'Repeat prescriptions', 'Flu & Covid jabs', 'Weight loss clinic', 'Blood pressure checks', 'Travel vaccinations', 'Contraception', 'Free delivery over £15'] as $label)
                        <span class="h-1 w-1 shrink-0 rounded-full bg-brand-moss"></span>
        ...
        <div class="mx-auto max-w-[1280px] px-6 sm:px-8">
            ...
            <div class="mt-14 grid gap-6 lg:grid-cols-3" data-reveal-group>
                ...
                @foreach ([
                    ...
                    ['href' => route('shop'), 'chip' => 'Home delivery', 'tone' => 'quiet', 'title' => 'Online shop', 'body' => 'Essentials delivered, free over '.config('pharmacy.free_delivery_threshold').'. Vitamins, remedies and everyday healthcare.', 'img' => 'parcel-doorstep.jpg', 'cta' => 'Shop essentials'],
                ] as $card)
                    <a href="{{ $card['href'] }}"
            ...
            <div class="reveal mt-6 rounded-[20px] border border-brand-hairline bg-white p-8 shadow-editorial-card">
                <div class="grid gap-x-12 sm:grid-cols-2">
                    @foreach ([
                        ...
                        <a href="{{ $href }}"
                            class="group -mx-3 flex items-center justify-between gap-4 rounded-xl border-b border-brand-hairline px-3 py-3.5 transition-colors duration-200 last:border-0 hover:bg-brand-ivory focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-moss sm:[&:nth-last-child(2)]:border-0">
                            <span class="text-sm text-brand-forest transition-colors duration-200 group-hover:text-brand-moss">{{ $name }}</span>
                            <span class="flex shrink-0 items-center gap-1.5 text-right text-xs text-brand-stone-light">
                                {{ $note }}
                                <svg class="h-3.5 w-3.5 shrink-0 -translate-x-1 text-brand-moss opacity-0 transition-all duration-200 group-hover:translate-x-0 group-hover:opacity-100" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
                            </span>
                        </a>
                    @endforeach
                </div>
                <p class="mt-6 text-center text-sm text-brand-stone-light">
                    <a href="{{ route('services') }}" class="font-serif italic underline decoration-brand-hairline underline-offset-4 transition hover:text-brand-forest hover:decoration-brand-moss">See all 19 services →</a>
                </p>
            </div>
        </div>
    </section>
    ...
    <section class="no-interact border-y border-brand-pistachio-line bg-brand-pistachio py-24 md:py-32">
        <div class="mx-auto max-w-[1280px] px-6 text-center sm:px-8">
            ...
            <x-steps :items="[
                ['title' => 'Nominate us', 'body' => 'Choose any Hollytree Pharmacy branch in the NHS App, or let our team set it up for you.'],
                ...
                <div class="grid gap-3 sm:grid-cols-2 lg:col-span-7" data-reveal-group>
                    @foreach ($branches as $branch)
                        <a href="{{ route('branches.show', $branch['slug']) }}" data-branch="{{ $branch['slug'] }}" data-town="{{ $branch['town'] }}" data-lat="{{ $branch['lat'] }}" data-lon="{{ $branch['lon'] }}"
                            class="reveal group flex flex-col rounded-2xl border bg-white p-5 shadow-editorial-card transition-all duration-[250ms] hover:-translate-y-1 hover:border-brand-moss hover:shadow-editorial-hover focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-moss {{ $branch['hub'] ? 'border-brand-moss/40' : 'border-brand-hairline' }}">
                            ...
                            <div class="mt-auto flex items-center justify-between gap-2 pt-3">
                                <p class="flex items-center gap-1.5 text-xs font-medium text-brand-moss">
                                    <span class="h-1.5 w-1.5 rounded-full bg-emerald-500"></span>
                                    Open until {{ $branch['closes'] }}
                                </p>
                                <span class="text-brand-stone-light transition-transform duration-200 group-hover:translate-x-1 group-hover:text-brand-moss">→</span>
                            </div>
                        </a>
                    @endforeach
                </div>
                    ...
                    <figure class="lg:sticky lg:top-28">
                        <div class="relative overflow-hidden rounded-[28px] border border-brand-hairline shadow-editorial-card">
                            <div id="branch-map" class="h-[420px] w-full bg-brand-hairline lg:h-[560px]">
                                ...
                                <img data-map-fallback src="{{ asset('images/branches-map.jpg') }}" width="970" height="1303" loading="lazy"
                                    alt="Map of the West Midlands showing all six Hollytree Pharmacy branches"
                        ...
                        <div class="mt-3 flex flex-wrap items-center justify-between gap-2">
                            ...
                            <button type="button" data-map-reset
                                class="rounded-full border border-brand-hairline px-3 py-1.5 text-[11px] font-semibold text-brand-forest transition hover:border-brand-forest hover:bg-brand-forest hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-moss">
                                Reset map
                            </button>
                        </div>
                    </figure>
        ...
        <div class="mx-auto max-w-[1280px] px-6 sm:px-8">
            <div class="reveal mx-auto max-w-2xl text-center">
                ...
                <p class="no-interact mt-5 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-base text-brand-stone">
                    ...
                    <span><span class="font-semibold text-brand-forest">{{ $reviews['count'] }} Google reviews</span> across our six pharmacies</span>
                </p>
            </div>
            ...
            <div class="reveal mt-14 grid gap-10 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,0.9fr)] lg:items-center lg:gap-14" data-testimonials>
                ...
                <div class="relative">
                    ...
                    <div aria-hidden="true" class="absolute inset-0 -translate-x-3.5 translate-y-3.5 -rotate-[1.3deg] rounded-[28px] border border-brand-pistachio-line bg-brand-pistachio/60"></div>
                    ...
                    <div class="relative grid rounded-[28px] border border-brand-hairline bg-white p-8 shadow-editorial-card sm:p-10">
                        @foreach (config('pharmacy.reviews_quotes') as $i => $q)
                            <figure id="testimonial-{{ $i }}" data-testimonial="{{ $i }}" role="tabpanel"
                                ...
                                <div class="flex items-center gap-3.5">
                                    <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-brand-forest font-serif text-xl text-white">{{ $q['initial'] }}</span>
                                    <span class="min-w-0">
                                        <span class="block truncate text-[15px] font-semibold text-brand-forest">{{ $q['name'] }}</span>
                                        <span class="flex gap-0.5" aria-label="{{ $q['rating'] }} out of 5 stars">
                                            @for ($star = 1; $star <= 5; $star++)
                                                <svg class="h-3.5 w-3.5 {{ $star <= $q['rating'] ? 'text-brand-moss' : 'text-brand-hairline' }}" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path d="M10 1.5l2.6 5.3 5.9.9-4.2 4.1 1 5.8-5.3-2.8-5.3 2.8 1-5.8L1.5 7.7l5.9-.9z"/></svg>
                                            @endfor
                                        </span>
                                    </span>
                                </div>
                                <blockquote class="my-auto py-7 font-serif text-[20px] italic leading-relaxed text-brand-forest sm:text-[23px]">“{{ $q['quote'] }}”</blockquote>
                ...
                <div>
                    <h3 class="font-serif text-2xl text-brand-forest">Patient testimonies</h3>
                    ...
                    <div class="tm-scroll mt-5 pr-2" role="tablist" aria-label="Patient testimonies" aria-orientation="vertical">
                        @foreach (config('pharmacy.reviews_quotes') as $i => $q)
                            <button type="button" id="testimonial-tab-{{ $i }}" data-testimonial-tab="{{ $i }}"
                                ...
                                class="tm-tab group flex w-full items-center gap-3 rounded-2xl border border-transparent px-3 py-2.5 text-left transition-all duration-200 hover:bg-brand-ivory focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-moss {{ $loop->first ? 'is-active' : '' }}">
                                <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-forest font-serif text-sm text-white">{{ $q['initial'] }}</span>
                                <span class="min-w-0 flex-1">
                                    <span class="block truncate text-sm font-semibold text-brand-forest">{{ $q['name'] }}</span>
                                    <span class="flex gap-0.5" aria-hidden="true">
                                        @for ($star = 1; $star <= 5; $star++)
                                            <svg class="h-3 w-3 {{ $star <= $q['rating'] ? 'text-brand-moss' : 'text-brand-hairline' }}" viewBox="0 0 20 20" fill="currentColor"><path d="M10 1.5l2.6 5.3 5.9.9-4.2 4.1 1 5.8-5.3-2.8-5.3 2.8 1-5.8L1.5 7.7l5.9-.9z"/></svg>
                                        @endfor
                                    </span>
                                </span>
                            </button>
            ...
            <div class="reveal mt-12 flex justify-center">
                <a href="{{ config('pharmacy.google_reviews_url') }}" target="_blank" rel="noopener" data-magnetic
                    class="group inline-flex items-center gap-2 rounded-full border border-brand-forest px-7 py-3.5 text-sm font-semibold text-brand-forest transition-all duration-200 hover:bg-brand-forest hover:text-white active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-moss">
        ...
        <div class="mx-auto max-w-[720px] px-6 sm:px-8">
            ...
            <p class="reveal mt-7 text-center text-[13px] text-footer-meta">
                ...
                <a href="{{ config('pharmacy.phone_href') }}"
                    class="text-footer-link underline decoration-footer-link/40 underline-offset-4 transition hover:text-white hover:decoration-white">{{ config('pharmacy.phone') }}</a>
                or email
                <a href="mailto:{{ config('pharmacy.email') }}"
                    class="text-footer-link underline decoration-footer-link/40 underline-offset-4 transition hover:text-white hover:decoration-white">{{ config('pharmacy.email') }}</a>
            </p>
        </div>
    </section>
