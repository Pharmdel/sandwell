@php
    ...
    $btnPrimary = $btn.' bg-brand-orange text-white hover:-translate-y-0.5 hover:bg-brand-orange-hover hover:shadow-orange-glow focus-visible:ring-brand-orange';
    $btnOutline = $btn.' border border-brand-navy text-brand-navy hover:bg-brand-navy hover:text-white focus-visible:ring-brand-navy';
    $btnLight = $btn.' border border-white/25 text-white hover:bg-white hover:text-brand-navy focus-visible:ring-white';
    ...
    $opt = 'wl-opt flex w-full flex-col items-start rounded-2xl border border-brand-hairline bg-white px-5 py-4 text-left text-[15px] font-medium text-brand-navy hover:border-brand-navy/50';
...
<x-layout title="Weight loss clinic West Bromwich | Mounjaro & Wegovy" hero description="Pharmacist-led weight loss with Wegovy, Mounjaro, Wegovy oral tablets and Alli — free suitability check, free consultation call, regular check-ins and free local delivery across six Sandwell branches.">
    ...
    <x-page-hero image="weightloss-outdoor.jpg" alt="A woman walking outdoors in morning light">
        <div class="max-w-3xl">
            <p class="reveal text-[11px] font-bold uppercase tracking-[0.14em] text-brand-orange">Weight loss clinic · West Bromwich</p>
        ...
        <div class="mx-auto max-w-[1280px] px-6 sm:px-8">
            ...
            <div class="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4" data-reveal-group>
                @foreach ($wl['treatments'] as $t)
                    <article class="reveal tilt relative flex flex-col rounded-[20px] border border-brand-hairline bg-white p-6 shadow-editorial-card {{ $t['badge'] ? 'ring-1 ring-brand-orange/40' : '' }}">
                        @if ($t['badge'])
                        ...
                        <ul class="mt-4 space-y-2 text-sm leading-relaxed text-brand-stone">
                            @foreach ($t['bullets'] as $b)
                                <li class="flex gap-2.5"><span class="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-brand-orange"></span>{{ $b }}</li>
                            @endforeach
                        </ul>
                        <div class="mt-auto flex flex-col gap-2 pt-6">
                            ...
                            <button type="button" data-modal-open="info-{{ $t['key'] }}" class="text-sm font-semibold text-brand-navy underline decoration-brand-hairline underline-offset-4 hover:decoration-brand-orange">More about {{ str($t['name'])->before('®') }}</button>
                        </div>
                    </article>
                @endforeach
            </div>
            ...
            <div class="reveal mt-10 text-center">
                <button type="button" data-modal-open="wl-checker" class="text-sm font-semibold text-brand-orange hover:underline">Not sure? Take the 1-minute suitability check →</button>
            </div>
        </div>
    </section>
        ...
        <div class="mx-auto max-w-[1280px] px-6 sm:px-8">
            <div class="grid gap-6 md:grid-cols-2" data-reveal-group>
                @foreach ($wl['outcomes'] as $o)
                    <div class="reveal rounded-[20px] border border-brand-peach-line bg-white p-8">
                        <p class="font-serif text-6xl text-brand-navy md:text-7xl">−<span data-count="{{ $o['figure'] }}">{{ $o['figure'] }}</span> <span class="text-3xl text-brand-stone-light">lbs</span></p>
                        <p class="mt-3 text-[11px] font-semibold uppercase tracking-[0.1em] text-brand-orange">{{ $o['when'] }}</p>
                        <p class="mt-1 text-lg text-brand-navy">{{ $o['text'] }}</p>
                    </div>
                @endforeach
            </div>
                ...
                @foreach ($wl['journey'] as $i => $s)
                    <div class="reveal rounded-[20px] border border-brand-hairline bg-white p-7 shadow-editorial-card">
                        <div class="flex items-center justify-between">
                            <span class="font-serif text-4xl text-brand-orange">{{ str_pad($i + 1, 2, '0', STR_PAD_LEFT) }}</span>
                ...
                @foreach ($wl['testimonials'] as $q)
                    <figure class="reveal flex flex-col rounded-[20px] border border-brand-hairline bg-brand-ivory p-7">
                        <div class="flex items-center gap-3">
                            <span class="flex h-11 w-11 items-center justify-center rounded-full bg-brand-navy text-sm font-semibold text-white">{{ $q['initial'] }}</span>
                            <span>
                                <span class="block text-sm font-semibold text-brand-navy">{{ $q['name'] }}</span>
                                <span class="flex gap-0.5 text-brand-orange" aria-label="Five stars">
                        ...
                        <p class="mt-4 text-sm font-bold text-brand-orange-hover">{{ $q['result'] }}</p>
                        <blockquote class="mt-2 font-serif text-lg italic leading-relaxed text-brand-navy">“{{ $q['quote'] }}”</blockquote>
    ...
    <section id="switch" class="scroll-mt-28 bg-brand-ivory py-24 md:py-32">
        <div class="mx-auto grid max-w-[1280px] items-center gap-12 px-6 sm:px-8 lg:grid-cols-2">
            <div class="reveal">
                ...
                <ul class="mt-6 space-y-3">
                    @foreach ($wl['switch']['bullets'] as $b)
                        <li class="flex items-center gap-3 text-[15px] font-medium text-brand-navy">
            ...
            <div class="reveal rounded-[24px] border border-brand-hairline bg-white p-8 text-center shadow-editorial-card md:p-10">
                <p class="font-serif text-3xl text-brand-navy">Switch in <span class="editorial-highlight">2 minutes.</span></p>
            ...
            <div class="mt-12 max-w-3xl divide-y divide-brand-hairline border-y border-brand-hairline" data-reveal-group>
                @foreach ($wl['faqs'] as [$q, $a])
                    <details class="wl-faq reveal group py-1">
                        <summary class="flex items-center justify-between gap-6 py-5 text-lg font-medium text-brand-navy">
                            {{ $q }}
    ...
    <section class="border-t border-brand-hairline bg-white py-16">
        <div class="mx-auto grid max-w-[1280px] gap-8 px-6 text-sm sm:px-8 md:grid-cols-3">
            <div class="reveal">
                ...
                <p class="mt-2 text-brand-navy">GPhC-registered pharmacist care. Regulated by the <a href="https://www.pharmacyregulation.org/registers/pharmacy" target="_blank" rel="noopener" class="font-semibold underline decoration-brand-hairline underline-offset-4 hover:decoration-brand-orange">General Pharmaceutical Council</a>.</p>
            </div>
            <div class="reveal">
                <p class="text-[11px] font-semibold uppercase tracking-[0.1em] text-brand-stone-light">Superintendent pharmacists</p>
                <p class="mt-2 text-brand-navy">One per operating company — <a href="{{ route('terms') }}#companies" class="font-semibold underline decoration-brand-hairline underline-offset-4 hover:decoration-brand-orange">named on each branch page</a>.</p>
            </div>
            <div class="reveal">
                <p class="text-[11px] font-semibold uppercase tracking-[0.1em] text-brand-stone-light">Registered pharmacies</p>
                <p class="mt-2 text-brand-navy">Six premises, each separately registered — <a href="{{ route('terms') }}#companies" class="font-semibold underline decoration-brand-hairline underline-offset-4 hover:decoration-brand-orange">see the full list</a>.</p>
            </div>
        </div>
    </section>
    ...
    <div id="wl-checker" class="wl-modal" hidden role="dialog" aria-modal="true" aria-labelledby="wl-checker-title" data-endpoint="{{ route('weight-loss.consultation') }}">
        <div class="absolute inset-0 bg-brand-navy-deep/60 backdrop-blur-sm" data-modal-close></div>
        <div class="wl-modal__box relative max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white p-6 shadow-editorial-hover sm:rounded-3xl sm:p-10">
            <button type="button" data-modal-close aria-label="Close" class="absolute right-4 top-4 flex h-10 w-10 items-center justify-center rounded-full bg-brand-ivory text-brand-navy hover:bg-brand-navy hover:text-white">
            ...
            <div data-body>
                <p class="text-[11px] font-bold uppercase tracking-[0.14em] text-brand-orange">Free suitability check</p>
                ...
                <div class="wl-step mt-7" data-step="bmi" hidden>
                    ...
                    <div data-hw hidden class="mt-4 rounded-2xl border border-brand-hairline bg-brand-ivory p-5">
                        <div class="flex gap-2" role="group" aria-label="Units">
                            <button type="button" data-unit="metric" aria-pressed="true" class="rounded-full border border-brand-navy px-4 py-1.5 text-xs font-semibold text-brand-navy aria-pressed:bg-brand-navy aria-pressed:text-white">cm / kg</button>
                            <button type="button" data-unit="imperial" aria-pressed="false" class="rounded-full border border-brand-navy px-4 py-1.5 text-xs font-semibold text-brand-navy aria-pressed:bg-brand-navy aria-pressed:text-white">ft &amp; in / st &amp; lb</button>
                        </div>
                        ...
                        <p data-bmi-out hidden aria-live="polite" class="mt-4 rounded-xl bg-white px-4 py-3 text-sm text-brand-navy"></p>
                    </div>
                </div>
                ...
                <div class="wl-step mt-7" data-step="result" hidden>
                    ...
                    <div data-book-cta class="mt-5">
                        ...
                        <button type="button" data-modal-open="wl-switch" class="mt-3 text-sm font-semibold text-brand-orange hover:underline">Already on treatment from another provider? Switch to us instead →</button>
                    </div>
                    <form data-booking hidden class="mt-6 space-y-4" onsubmit="return false">
                        ...
                        <label class="flex items-start gap-3 rounded-xl bg-brand-ivory p-4 text-sm leading-relaxed text-brand-stone">
                            <input type="checkbox" name="scr_consent" class="mt-1 h-4 w-4 rounded border-brand-hairline accent-brand-orange">
                            <span>I consent to the Sandwell Pharmacy Group pharmacist accessing my <strong class="text-brand-navy">NHS Summary Care Record</strong> to check my medication history before supplying treatment.</span>
                        </label>
        ...
        <div id="info-{{ $t['key'] }}" class="wl-modal" hidden role="dialog" aria-modal="true" aria-labelledby="info-{{ $t['key'] }}-title">
            ...
            <div class="wl-modal__box relative max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white p-6 shadow-editorial-hover sm:rounded-3xl sm:p-10">
                ...
                <div class="grid gap-6 sm:grid-cols-[180px_1fr] sm:items-start">
                    ...
                    <div>
                        <p class="text-[11px] font-semibold uppercase tracking-[0.1em] text-brand-orange">{{ $t['tag'] }}</p>
                ...
                <div class="mt-6 space-y-4 text-[15px] leading-relaxed text-brand-stone">
                    @foreach ($t['detail'] as [$h, $p])
                        <p><strong class="text-brand-navy">{{ $h }}:</strong> {{ $p }}</p>
                    @endforeach
                </div>
                <p class="mt-6 text-[11px] font-semibold uppercase tracking-[0.1em] text-brand-stone-light">Available strengths</p>
                <div class="mt-2 flex flex-wrap gap-2">
                    @foreach ($t['strengths'] as $s)<span class="rounded-full border border-brand-hairline bg-brand-ivory px-3 py-1 text-xs font-medium text-brand-navy">{{ $s }}</span>@endforeach
                </div>
        ...
        <div class="wl-modal__box relative max-h-[94vh] w-full max-w-xl overflow-y-auto rounded-t-3xl bg-white p-6 shadow-editorial-hover sm:rounded-3xl sm:p-10">
            ...
            <div data-body>
                ...
                <form id="wl-waitlist-form" action="{{ route('weight-loss.waitlist') }}" method="post" class="mt-6 space-y-4">
                    ...
                    <p data-fdy-bmi aria-live="polite" class="rounded-xl bg-brand-ivory px-4 py-3 text-sm text-brand-navy">Enter your height and weight and we’ll work out your BMI.</p>
