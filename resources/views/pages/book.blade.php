...
<x-layout title="Book an appointment" hero description="Book a flu jab, Covid-19 vaccination, Pharmacy First consultation or blood pressure check at any Hollytree Pharmacy branch.">
    <x-page-hero image="flu-vaccine.jpg" alt="A pharmacist preparing a vaccination" size="short">
        <div class="max-w-3xl">
            <p class="reveal text-[11px] font-bold uppercase tracking-[0.14em] text-brand-moss">Book online · About 2 minutes</p>
                ...
                @foreach ($steps as $i => $step)
                    <li class="flex items-center gap-3">
                        <span @class([
                            ...
                            'bg-brand-pistachio text-brand-moss' => $i === 0,
                            'bg-brand-forest/5 text-brand-stone-light' => $i !== 0,
                        ])>{{ $i + 1 }}</span>
                        <span @class([
                            'text-sm',
                            'font-semibold text-brand-forest' => $i === 0,
                    ...
                    <div class="rounded-3xl border border-brand-hairline bg-white p-8 shadow-editorial-card md:p-10">
                        ...
                        <div class="mt-8 grid gap-4 sm:grid-cols-2">
                            @foreach ($services as $i => $service)
                                <label class="group relative flex cursor-pointer flex-col rounded-[20px] border border-brand-hairline p-6 transition-all duration-200 hover:border-brand-forest/40 has-[:checked]:border-2 has-[:checked]:border-brand-forest">
                                    <input type="radio" name="service" value="{{ $service['value'] }}" @checked($i === 0)
                                        class="peer sr-only">
                                    <span class="absolute right-5 top-5 hidden h-5 w-5 items-center justify-center rounded-full bg-brand-moss text-white peer-checked:flex">
                                    ...
                                    <span class="flex h-11 w-11 items-center justify-center rounded-full bg-brand-pistachio">
                                        <svg class="h-5 w-5 text-brand-moss" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.7" d="M12 6v12m6-6H6"/></svg>
                                    </span>
                                    <span class="mt-4 block text-lg font-semibold text-brand-forest">{{ $service['title'] }}</span>
                            ...
                            <div class="mt-5">
                                @foreach ([['Service', 'Flu vaccination'], ['Eligibility', 'Next step'], ['Branch', 'Not chosen'], ['Time', 'Not chosen']] as $row)
                                    <div class="flex items-center justify-between gap-3 border-b border-brand-hairline py-3 last:border-0">
                                        ...
                                        <span class="text-sm text-brand-forest">{{ $row[1] }}</span>
                                    </div>
                                @endforeach
                            </div>
                            <p class="mt-5 rounded-2xl bg-brand-pistachio p-4 text-xs leading-relaxed text-brand-moss-hover">
                            ...
                            <div class="mt-4 space-y-3 opacity-50">
                                @foreach (['Your age group', 'Pregnancy or long-term condition', 'Recent vaccinations'] as $q)
                                    <div class="flex items-center justify-between gap-3 border-b border-brand-hairline pb-3 last:border-0">
                                        ...
                                        <span class="rounded-full bg-brand-forest/5 px-3 py-1 text-[10px] uppercase tracking-wide text-brand-stone-light">Step 2</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
    ...
    <section class="no-interact border-t border-brand-pistachio-line bg-brand-pistachio py-20">
